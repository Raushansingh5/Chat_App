    ...
    <div className='min-h-screen bg-cover bg-no-repeat flex items-center justify-center'>
      <div className='w-5/6 max-w-2xl backdrop-blur-2xl text-gray-300 border-2 border-gray-600 flex items-center justify-between max-sm:flex-col-reverse rounded-lg'>
        <form onSubmit={handleSubmit} className='flex flex-col gap-5 p-10 flex-1'>
          ...
          <input  value={name} onChange={(e)=>setName(e.target.value)} type="text" required placeholder='Your Name' className='p-2 border border-gray-500 rounded-md  bg-transparent focus-outline-none focus:right-2 focus:ring-violet-500' />
          <textarea value={bio} onChange={(e)=>setBio(e.target.value)} required placeholder='Write Profile Bio' className='p-2 border border-gray-500 rounded-md  bg-transparent focus-outline-none focus:right-2 focus:ring-violet-500'></textarea>
